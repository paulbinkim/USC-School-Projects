...
void lcd_init(void)
{
    DDRD |= DATA_BITS; // Data bits
    DDRB |= CTRL_BITS; // Set the DDR register bits for ports B and D to output (1) // Control bits
...
*/
// Mid-Level Function that deals with sending data and commands to the LCD by making use of the low level functions.
...
// Low Level Function (Handle the changing of bits in the I/O ports connected to the LCD. Most of the bit fiddiling is done here)
// Primary task will be sending a 4-bit nibble to the LCD by making use of the low level functions.
